...
class TestDetectHttpVersionSupport:
    ...
    def test_detect_http2_support(self, mock_ssl_context: Mock, mock_socket: Mock) -> None:
    ...
    @patch('modules.header_checks.http_version.socket.create_connection')
    @patch('modules.header_checks.http_version.ssl.create_default_context')
    def test_detect_http11_support(self, mock_ssl_context: Mock, mock_socket: Mock) -> None:
...
class TestRiskBadge:
    ...
    def test_risk_badge_for_risky_item(self) -> None:
        ...
        item = {
            "accepted": True,
            "flags": ["empty_version", "invalid_token"]
        }
...
class TestErrorHandling:
    ...
    def test_check_http_version_network_error(self, mock_requests: Mock, capsys: pytest.CaptureFixture[str]) -> None:
