...
class TestPriority1:
    ...
    def test_cli_help_command(self, hexhttp_command: Any) -> None:
        ...
        assert 'usage:' in result['stdout'].lower() or 'hexhttp' in result['stdout'].lower()
    ...
    def test_apache_detection(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        assert assert_technology_detected(output, 'apache'), \
    ...
    def test_nginx_detection(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        output = result['stdout'] + result['stderr']
        assert assert_technology_detected(output, 'nginx'), \
    ...
    def test_cloudflare_detection(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        output = result['stdout'] + result['stderr']
        assert assert_technology_detected(output, 'cloudflare'), \
            f"Cloudflare not detected in output: {output}"
    
    @pytest.mark.priority1
    def test_safe_endpoint_no_false_positives(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        output = result['stdout'] + result['stderr']
        assert assert_no_false_positives(output), \
...
class TestPriority2:
    ...
    def test_file_input_processing(self, hexhttp_command: Any, temp_url_file: Any) -> None:
    ...
    @pytest.mark.priority2
    def test_threading_stability(self, hexhttp_command: Any, temp_url_file: Any) -> None:
    ...
    def test_header_analysis(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        # Use quick mode for faster testing
        result = hexhttp_command('-u', test_urls['uncommon_headers'], '--ocp', timeout=10)
        ...
        found_header_analysis = any(indicator in output.lower() for indicator in header_indicators)
    ...
    def test_cache_file_detection(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        found_cache_analysis = any(indicator in output.lower() for indicator in cache_indicators)
...
class TestPriority3:
    ...
    def test_verbose_output(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        result_verbose = hexhttp_command('-u', test_urls['safe'], '-v', '--ocp', timeout=10)
    ...
    @pytest.mark.priority3
    @pytest.mark.slow
    def test_hho_vulnerability_detection(self, hexhttp_command: Any, test_urls: Any) -> None:
...
class TestIntegration:
    ...
    def test_complete_scan_workflow(self, hexhttp_command: Any, test_urls: Any) -> None:
        ...
        result = hexhttp_command(
            '-u', test_urls['apache'],
            '-v',  # verbose
            '-H', 'X-Test: integration-test',  # custom header
            '--ocp', timeout=15
        )
    ...
    @pytest.mark.integration
    @pytest.mark.slow
    def test_multi_url_complete_workflow(self, hexhttp_command: Any, temp_url_file: Any) -> None:
...
class TestUtilities:
    ...
    def test_unreachable_url_handling(self, hexhttp_command: Any) -> None:
        ...
        result = hexhttp_command('-u', 'http://192.0.2.1:12345/')  # RFC5737 test address
