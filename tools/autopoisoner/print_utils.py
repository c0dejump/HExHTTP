...
def behavior_or_confirmed_message(uri, behaviorOrConfirmed, behaviorType, explicitCache, url, status_codes="default", header = "default", outputFile = "default", LOCK = "default"):
    ...
    if header != "default":
        message = f" └── {messageDict[behaviorOrConfirmed]} | {messageDict[behaviorType]} | EXPLICIT CACHE : {explicitCache} | \033[34m{uri}\033[0m | PAYLOAD : {header}"
        print(message)
        if behaviorOrConfirmed != "BEHAVIOR":
            vuln_found_notify(uri, header)
    else:
        message = f" └── {messageDict[behaviorOrConfirmed]} | PORT {messageDict[behaviorType]} | EXPLICIT CACHE : {explicitCache} | \033[34m{uri}\033[0m | PAYLOAD : {header}"
        print(message)
        if behaviorOrConfirmed != "BEHAVIOR":
            vuln_found_notify(uri, header)
