...
def verify_cache_poisoning(VULN_TYPE: str, conn: http.client.HTTPConnection | http.client.HTTPSConnection, url: str, payload: str, main_status_code: int, authent: tuple[str, str] | None, host: str) -> None:
    cb = random.randrange(9999)
    res_status = 0
    try:
        for _ in range(5):
            conn.putrequest("GET", f"/?CPDoS={cb}") #  discrepancy between conn and req : url endpoint isn't reflected in the putrequest
...
def duplicate_headers(conn: http.client.HTTPConnection | http.client.HTTPSConnection, url: str, mh: str, main_status_code: int, authent: tuple[str, str] | None) -> tuple:
    #VULN_TYPE = "DH"
    cb = random.randrange(9999)
...
def referer_duplicate_headers(conn: http.client.HTTPConnection | http.client.HTTPSConnection, url: str, main_status_code: int, authent: tuple[str, str] | None) -> tuple:
    #VULN_TYPE = "RDH"
    cb = random.randrange(9999)
...
def host_duplicate_headers(conn: http.client.HTTPConnection | http.client.HTTPSConnection, host: str, url: str, main_status_code: int, authent: tuple[str, str] | None) -> tuple:
    #VULN_TYPE = "HDH"
    cb = random.randrange(9999)
...
def MHC(url: str, req_main: requests.Response, authent: tuple[str, str] | None, human: str) -> None:
    ...
    try:
        ...
        for vuln_type in mhc_res:
            ...
            print("\033[K", end="")
            if vuln_type_res and vuln_type_res is not None and isinstance(vuln_type_res, tuple):
