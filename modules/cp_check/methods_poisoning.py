...
def verify_fat_get_poisoning(
    ...
    if req_verify.status_code == req_main_check.status_code and req_main_check.status_code != req_main.status_code:
        print_result(Identify.confirmed, "FAT", f"{rm}", "DIFFERENT STATUS-CODE", f"{req_main.status_code} > {req_verify.status_code}", url, d)
    elif len(req_verify.content) == len(req_main_check.content) and len(req_main_check.content) != len_main:
        print_result(Identify.confirmed, "FAT", f"{rm}", "DIFFERENT RESP LENGTH", f"{len_main}b > {len(req_verify.content)}b", url, d)
        ...
        print_result(Identify.confirmed, "FAT",  f"{rm}", "HEADERS REFLECTION", "", url, d)
    ...
    for d in body_datas:
        for rm in requests_method:
            ...
            if req_fg.status_code != req_main.status_code:
                print_result(Identify.behavior, "FAT" ,f"{rm}", "DIFFERENT STATUS-CODE", f"{req_main.status_code} > {req_fg.status_code}", url, d)
                behavior_check = True
                verify_fat_get_poisoning(s, url, d, rm, req_main, len_main, authent)
            elif len_fg not in range_exlusion:
                print_result(Identify.behavior, "FAT", f"{rm}", "DIFFERENT RESP LENGTH", f"{req_main.status_code}b > {len_fg}b", url, d)
                behavior_check = True
                verify_fat_get_poisoning(s, url, d, rm, req_main, len_main, authent)
            elif d in req_fg.text or "codejump" in req_fg.text:
                print_result(Identify.behavior, "FAT", f"{rm}", "BODY REFLECTION", "", url, d)
                behavior_check = True
                verify_fat_get_poisoning(s, url, d, rm, req_main, len_main, authent)
            elif d in req_fg.headers or "codejump" in req_fg.headers:
                print_result(Identify.behavior, "FAT", f"{rm}", "HEADERS REFLECTION", "", url, d)
                behavior_check = True
                verify_fat_get_poisoning(s, url, d, rm, req_main, len_main, authent)
            ...
            if rm == "POST":
                ...
                for d in body_datas:
                    ...
                    if req_mix.status_code != req_get.status_code:
                        print_result(Identify.behavior, "MIX", f"{rm} <> GET", "DIFFERENT STATUS-CODE", f"{req_main.status_code} > {req_mix.status_code}", url, d)
                        behavior_check = True
                    elif len(req_mix.content) not in range_exlusion and len(req_mix.content) != len(req_get.content):
                        print_result(Identify.behavior, "MIX", f"{rm} <> GET", "DIFFERENT RESP LENGTH", f"{len(req_main.content)}b > {len(req_mix.content)}b", url, d)
                        behavior_check = True
                    if behavior_check and proxy.proxy_enabled:
                        from utils.proxy import proxy_request
