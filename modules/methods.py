...
def get(url): req_p = requests.get(url, verify=False, allow_redirects=False, headers=header, timeout=10); return req_p.status_code, req_p.headers, "GET", len(req_p.content), req_p.content
...
def put(url): req_pt = requests.put(url, verify=False, allow_redirects=False, headers=header, timeout=10); return req_pt.status_code, req_p.headers, "PUT", len(req_pt.content), req_p.content
def patch(url): req_ptch = requests.patch(url, verify=False, allow_redirects=False, headers=header, timeout=10); return req_ptch.status_code, req_p.headers, "PATCH", len(req_ptch.content), req_p.content
def options(url): req_o = requests.options(url, verify=False, allow_redirects=False, headers=header, timeout=10); return req_o.status_code, req_p.headers, "OPTIONS", len(req_o.content), req_p.content
def trace(url): req_o = requests.trace(url, verify=False, allow_redirects=False, headers=header, timeout=10); return req_o.status_code, req_p.headers, "TRACE", len(req_o.content), req_p.content
...
def check_methods(url, custom_header, authent):
    ...
    try:
        ...
        print(f" └── HELP{'':<4}: {rs:<3} [{len_req} bytes] [Cacheable: {cache_status}]")
        #print(resp.data)
    ...
    try:
        ...
        for rh in resp_h:
            ...
            or "x-nextjs-cache" in rh:
                cache_status = True
        len_req = len(resp.data.decode('utf-8'))
        print(f" └── PURGE{'':<3}: {rs:<3} [{len_req} bytes] [Cacheable: {cache_status}]")
    except requests.packages.urllib3.exceptions.MaxRetryError as e:
    ...
    try:
        ...
        for rh in resp_h:
            ...
            or "x-nextjs-cache" in rh:
                cache_status = True
        len_req = len(resp.data.decode('utf-8'))
        print(f" └── DEBUG{'':<3}: {rs:<3} [{len_req} bytes] [Cacheable: {cache_status}]")
        #print(resp.data)
    except requests.packages.urllib3.exceptions.MaxRetryError as e:
    ...
    try:
        ...
        for rh in resp_h:
            ...
            or "x-nextjs-cache" in rh:
                cache_status = True
        len_req = len(resp.data.decode('utf-8'))
        print(f" └── PLOP{'':<4}: {rs:<3} [{len_req} bytes] [Cacheable: {cache_status}]")
        #print(resp.data)
    except requests.packages.urllib3.exceptions.MaxRetryError as e:
