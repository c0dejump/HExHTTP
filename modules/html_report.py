...
.report-footer { max-width:1200px; margin:2rem auto 0; padding:1.5rem; border-top:1px solid var(--border); display:flex; justify-content:space-between; font-size:0.7rem; color:var(--text-dim); }
...
function applyFilters(){
  ...
  document.querySelectorAll('.url-card').forEach(card=>{
    ...
    if(cardVisible&&hasFindingFilter&&findingCards.length>0){
      let anyMatch=false;
      ...
      // No finding filter active — show all findings
