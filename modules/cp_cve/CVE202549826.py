...
def valid_cache(uri: str, req: requests.Response, req_h: requests.Response, headers: dict) -> None:
    for _ in range(5):
    ...
    if req_confirm.status_code == req_verify.status_code and req_confirm.status_code != req.status_code and req_confirm.status_code == 204:
        print(f" {Identify.confirmed} | CVE-2025-49826 | {Colors.BLUE}{uri}{Colors.RESET} | {req.status_code} > {req_h.status_code} | PAYLOAD: {headers}")
    elif req_confirm.status_code == req_verify.status_code and req_confirm.status_code != req.status_code:
        print(f" {Identify.behavior} ++ | CVE-2025-49826 | {Colors.BLUE}{uri}{Colors.RESET} | {req.status_code} > {req_h.status_code} | PAYLOAD: {headers}")
    else:
        print(f" {Identify.behavior} | CVE-2025-49826 | {Colors.BLUE}{uri}{Colors.RESET} | {req.status_code} > {req_h.status_code} | PAYLOAD: {headers} ")


def nextjs_204(url: str) -> None:
